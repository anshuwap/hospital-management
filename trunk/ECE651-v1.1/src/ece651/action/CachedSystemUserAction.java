...
public class CachedSystemUserAction extends ActionSupport implements SessionAware, RequestAware 
{
	...
	public String CreateSystemUser()
	{
		...
		SystemUserService userService = new SystemUserService(); //init Cached SystemUserService
	...
	public String SearchSystemUser(){
		...
		try
		{
			if (isSearchAll)
				...
				retrieveSystemUsers = userService.searchAllUser();
			else if (!searchContent.isEmpty())
			{
				...
				if (searchType.compareTo("userName") == 0) //search by user name
					...
					tempSysUser =  userService.searchUserByUsername(searchContent);
				else if (searchType.compareTo("userId") == 0) //search by user id
				{
					...
					tempSysUser = userService.searchUserBySystemUserId(userId);
				}
				else
				{
		...
		{
		    this.setOperationStatus("Search System User Failed: Exception Happened: "+e.getMessage());
		    //systemUserDao.cleanup();
			return ERROR;
		}
